...
class FG_eval {
  ...
  public:
    ...
    void operator()(ADvector& fg, const ADvector& x) {
      ...
      double W_delta;
      double W_a;
      double W_ddelta; // weight cost for high difference between consecutive steering actuations
      double W_da; // weight cost for high difference between consecutive acceleration actuations

      if (!ros::param::get("/control/W_cte", W_cte) ||
      !ros::param::get("/control/W_epsi", W_epsi) ||
      !ros::param::get("/control/W_v", W_v) ||
      !ros::param::get("/control/W_delta", W_delta) ||
      !ros::param::get("/control/W_a", W_a) ||
      !ros::param::get("/control/W_ddelta", W_ddelta) ||
      !ros::param::get("/control/W_da", W_da)) {
          ROS_ERROR_STREAM("Failed to read weight parameters");
          ros::shutdown();
      }
      ...
      for (int i = 0; i < N; ++i) {
        ...
        const auto v = x[ID_FIRST_v + i] - target_speed;
      ...
      for (int i = 0; i < N - 1; ++i) {
        ...
        const auto psi1_f = psi0 + v0 * tan(delta0) / Lf * dt; // psi0 + v0 * (-delta0) / Lf * dt;
        const auto v1_f = v0 + a0 * dt;
        const auto cte1_f = py_desired - py0 + v0 * CppAD::sin(epsi0) * dt;
        const auto epsi1_f = psi0 - psi_desired + v0 * tan(delta0) / Lf * dt; // psi0 - psi_desired + v0 * (-delta0) / Lf * dt;
...
MPC::MPC() {
  ...
  float deltaHigh, deltaLow;
  if (!ros::param::get("/control/deltaHigh", deltaHigh) ||
  !ros::param::get("/control/deltaLow", deltaLow) ) {
      ROS_ERROR_STREAM("Failed to read delta limit parameters");
      ros::shutdown();
  }
  // all actuation inputs (steering, acceleration) should have values between [-1, 1]
  for (int i = ID_FIRST_delta; i < ID_FIRST_a; ++i) {
    this->x_lowerbound[i] = deltaLow;
    this->x_upperbound[i] = deltaHigh;
  }

  float aHigh, aLow;
  if (!ros::param::get("/control/aHigh", aHigh) ||
  !ros::param::get("/control/aLow", aLow) ) {
      ROS_ERROR_STREAM("Failed to read acceleration limit parameters");
      ros::shutdown();
  }
  for (int i = ID_FIRST_a; i < NX; ++i) {
    this->x_lowerbound[i] = aLow;
    this->x_upperbound[i] = aHigh;
  }
